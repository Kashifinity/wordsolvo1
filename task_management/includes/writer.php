...
<?php 
$retailer_account = "SELECT id FROM emp_login where user_role='writer' ";
$Total_emp = 0;
if ($result = mysqli_query($connection, $retailer_account)) {
    $Total_emp = mysqli_num_rows($result);
}

$retailer_account = "SELECT id FROM emp_login where user_role='writer' and status='1' ";
$Active_emp = 0;
if ($result = mysqli_query($connection, $retailer_account)) {
    $Active_emp = mysqli_num_rows($result);
}

$retailer_account = "SELECT id FROM emp_login where user_role='writer' and status='0' ";
$Deactive_emp = 0;
if ($result = mysqli_query($connection, $retailer_account)) {
...
?>
                    ...
                    <h6 class="element-header">Welcome to Writer Dashboard <?php echo $_SESSION['emp_name'];?></h6>
